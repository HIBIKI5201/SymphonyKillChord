...
{
    /// <summary>
    ///     常駐シーン初期化モジュールをフェーズ順に実行するCoordinatorです。
    /// </summary>
    public sealed class PersistentInitializationCoordinator
    {
        ...
        public async Awaitable<bool> InitializeAsync(
            IReadOnlyList<IPersistentInitializationModule> modules,
            ...
            CancellationToken cancellationToken)
        {
            for (int i = 0; i < modules.Count; i++)
            {
                ...
                IPersistentInitializationModule module = modules[i];
                if (!module.Init())
                {
        ...
        private static void LogPhaseFailure(
            IPersistentInitializationModule module,
            InitializationPhase phase)
        {
            Debug.LogError(
                $"[{nameof(PersistentInitializationCoordinator)}] " +
