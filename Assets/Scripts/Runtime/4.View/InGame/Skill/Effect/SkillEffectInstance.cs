...
namespace KillChord.Runtime.View.InGame.Skill.Effect
{
    ...
    public sealed class SkillEffectInstance : MonoBehaviour, ISkillEffectHandle
    {
        ...
        /// <summary> 発動時に適用するポストプロセスのConfigです。未設定なら演出しません。 </summary>
        public PostEffectOverlayConfig PostEffectConfig => _postEffectConfig;

        /// <summary> 発動からポストプロセスを掛け始めるまでの秒数です。 </summary>
        public float PostEffectDelaySeconds => _postEffectDelaySeconds;

        /// <summary> ポストプロセスを掛け続ける秒数です。0なら演出しません。 </summary>
        public float PostEffectSeconds => _postEffectSeconds;
        ...
        private const float DEFAULT_POST_EFFECT_DELAY_SECONDS = 0.1f;
        private const float DEFAULT_POST_EFFECT_SECONDS = 0.8f;
        ...
        [SerializeField]
        [Tooltip("発動時に適用するポストプロセスのConfigです。未設定なら演出しません。")]
        private PostEffectOverlayConfig _postEffectConfig;

        [SerializeField, Min(0f)]
        [Tooltip("発動からポストプロセスを掛け始めるまでの秒数です。")]
        private float _postEffectDelaySeconds = DEFAULT_POST_EFFECT_DELAY_SECONDS;

        [SerializeField, Min(0f)]
        [Tooltip("ポストプロセスを掛け続ける秒数です。0なら演出しません。")]
        private float _postEffectSeconds = DEFAULT_POST_EFFECT_SECONDS;
