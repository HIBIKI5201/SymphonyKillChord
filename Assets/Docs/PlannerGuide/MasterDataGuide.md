...
1. **ステージを作成する**
   ...
   - メニュー:
     - バトル: `KillChord/StageSelect/BattleStageAsset`
     - シナリオ: `KillChord/StageSelect/ScenarioStageAsset`
   - 主な設定項目:
     ...
     | 初回起動時に自動出撃するチュートリアルステージの場合はオンにします（Battleのみ） | ツリー全体で**1つのステージのみ**オンにできる（複数設定するとエラー） |
...
4. **ステージ間のBindを作成する**
   ...
   - メニュー: `KillChord/StageSelect/StageBindAsset`
   - `From Stage`と`To Stage`へ①で作ったステージを割り当てる
   - `Advance Mode`を選択する
