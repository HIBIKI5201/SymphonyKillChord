...
作戦画面のノードと接続線はUXMLへ個別に固定配置せず、`StageTreeAsset`から生成した`StageTree`をもとに実行時初期化で生成する。

- 入力Bindを持たないノードを起点として左端へ配置する。
- Bindのトポロジカル順に列番号を割り当て、後続ノードほど右側へ配置する。
- 同じ列に属する分岐ノードは上下へ並べる。
- グラフの高さが表示領域より小さい場合は、ノード群全体を画面の縦中央へ配置する。
- ノード数や列数が表示領域を超える場合はScrollViewで縦横に移動できるようにする。
- 起点が複数ある場合は左端へ複数配置し、`StageTreeAsset`の検証と画面初期化時に警告する。
- 起点が存在しない場合、またはBindに循環がある場合はエラーとして自動配置を中止する。

配置計算は`StageMapLayoutBuilder`が初期化時に一度だけ行う。ノード位置の更新を毎フレーム実行しない。
