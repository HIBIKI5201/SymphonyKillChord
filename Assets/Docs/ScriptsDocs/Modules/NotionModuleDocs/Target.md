...
| **モジュール名** | Target |
| **カテゴリ** | InGame |
| **アーキテクチャ** | クリーンアーキテクチャ（現状はAdaptor・View・Compositionのみで構成） |
| **ステータス** | 実装済み |
| **最終更新日** | 2026-07-15 |
...
### ✅ 設計上の見どころ
* **ロックオン機能の独立モジュール化**: かつてCamera内部にあったロックオン対象選択ロジックが独立したモジュールとして切り出されたことで、Player（攻撃対象解決）・Enemy（対象登録）・UI（体力表示）・Skill（ターゲット解決）という複数モジュールが同じ選択結果を共有できるようになりました。
* **`ITargetable`による疎結合な登録**: Enemyモジュールは`TransformTargetable`という薄いラッパーを介して自身を登録するだけでよく、Targetモジュール側はEnemyの具象型を一切知りません。

### ⚠️ 既知の課題・改善ポイント
* **Domain層の不在**: ターゲット選択の優先順位ロジック（内積・距離判定）が`TargetingSystem`（View層）に直接実装されており、Unityに依存しない単体テストが行いにくい構成になっています。
