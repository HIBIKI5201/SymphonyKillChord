...
### ✅ 設計上の見どころ
* **ルール駆動のナビゲーション履歴**: Android的な「Replace/Overlay/Reset」というルールをデータ（`ScreenRuleData`）側で定義できるため、新しい画面遷移パターンの追加や調整をコード変更なしで行えます。
* **コンテナと中身の責務分離**: Screenモジュールは「表示/非表示の器」のみを所有し、StageSelect/SkillTree等の機能モジュールが「中身」を構築するという分離により、機能モジュール同士が互いの画面構築コードに依存しません。

### ⚠️ 既知の課題・改善ポイント
* **`SkillBuildScreenView`の配置**: 内容的にはSkillモジュールに属する大きなViewが、物理的に`4.View/OutGame/Screen/`フォルダに置かれています。フォルダ構成とモジュール境界が一致していません。
* **`ScreenId`が2つのシーンにまたがる**: 1つのenumにOutGameシーンの画面とTitleシーンの画面が混在しており、実際には同時に使われることのない値が並んでいます。
* **初期化順序への暗黙の依存**: 各機能モジュールはScreenモジュールが先に初期化されコンテナが存在することを前提にしており、この依存関係は`Order`値の大小関係でのみ表現され、コード上に明記されていません。
