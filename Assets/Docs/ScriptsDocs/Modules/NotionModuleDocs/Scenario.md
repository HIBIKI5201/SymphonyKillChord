...
### ✅ 設計上の見どころ
* **テキストタイミングトリガーによる軽量な演出制御**: 分岐こそありませんが、`TextTimingTrigger`により文字送りの途中で立ち絵変化やフェードといった副次イベントを挟めるため、シンプルな線形データのままでも表現力のある演出が可能です。

### ⚠️ 既知の課題・改善ポイント
* **CSVパースの都度実行・キャッシュ無し**（`ScenarioCsvUtility`/`ScenarioRepository`）: `FindByIdAsync`を呼ぶたびにファイル全体を読み込みパースし直しており、`ScenarioDefinition`のキャッシュがありません。シナリオ開始時に毎回発生する読み込み負荷であり、過去のリファクタリングメモで指摘されていた懸念は現在も解消されていません。
* **文字送り中の文字列アロケーション**: `TextEventHandler`がテキストを1文字進めるたびに部分文字列を新規生成しており、表示速度によっては頻繁なGCアロケーションを引き起こす可能性があります。
