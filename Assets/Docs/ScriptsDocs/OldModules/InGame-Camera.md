...
### 1. Domain
- **CameraSystemParameter**: カメラの詳細設定。
    - オフセット（`Offset`）、距離（`Distance`）
    - 追従設定（`FollowOffsetPower`, `FollowLerpSpeed`）
    - 回転速度（`BoneRotateSpeed`, `FollowRotationSpeed`, `LockOnRotationSpeed`）
    - ロックオン挙動（`LockOnAngleMargin`, `LockOnLookAtRatio`）
    - 衝突判定（`CollisionRadius`, `CollisionMask`）
    - 制限と入力（`PitchRange`, `IsInvertVertical`, `IsInvertHorizontal`）
...
- **CameraFollowApplication**: 注視対象の移動速度に基づいた、注視点（`CameraCenterOffset`）の動的オフセット計算。
...
- **TargetSelector**: ロックオン対象の選択ロジック（カメラ前方との角度（Dot積）を優先し、負の場合は距離を優先）。
- **TargetManager**: 登録された全ロックオン対象のリスト管理。
- **TargetEntityRegistry**: ロックオン対象の `ILockOnTarget` と `CharacterEntity` の紐付け管理。
- **CameraSystemContext**: 1フレームの更新に必要なデータ（`FollowPosition`, `Input`, `MoveInput`, `DeltaTime`）を保持する `readonly ref struct`。

### 3. Adaptor
- **CameraSystemController**: View層からの要求（自動ロックオン開始、ロックオン切り替え）を Application へ伝達。
- **CameraSystemPresenter**: Application から算出された計算結果（位置・回転）を保持し、View が参照できるように仲介します。
- **TargetSelectorController**: ターゲット選択の要求や、現在のターゲットに対応するエンティティの取得を制御。
- **TargetEntityRegistryController**: ターゲットエンティティの登録・参照を制御。
- **LockOnTargetGateway**: `Transform` をラップして `ILockOnTarget` を実装するアダプター。
...
### 6. Composition
- **CameraSystemInitializer**: 各コンポーネントの生成、依存関係の注入、および `TargetSelectorController` の `ServiceLocator` への登録。
- **CameraSystemParameterDebug**: 実行中に GUI でパラメータを調整し、Domain モデルへ即時反映するためのデバッグ用コンポーネント。
- **CameraInitialize**: 永続的なカメラの `Transform` を `ICameraTransform` として `ServiceLocator` に登録するコンポーネント。
...
### 距離補正と衝突判定
- `Physics.SphereCast` を使用してカメラと注視点の間の遮蔽物を検知します（`GetDistance`）。
- 衝突が検知された場合、即座に距離を短縮して壁抜けを防止します。
- 衝突が解消された際は、`Mathf.Lerp` を用いて設定された `Distance` まで滑らかに復帰します（`UpdateDistance`）。

### 入力反転
- 垂直・水平方向の入力反転フラグ（`IsInvertVertical`, `IsInvertHorizontal`）をサポートしており、`CameraSystemApplication` 内で適用されます。
