...
namespace KillChord.Editor.AutoBuilder
{
    ...
    public class AutoBuilderProvider : SettingsProvider
    {
        ...
        public override void OnGUI(string searchContext)
        {
            ...
            if (AutoBuilderSettings.HasEmptyBuildProfile(AutoBuilderSettings.instance.MasterBuildProfiles)) { EditorGUILayout.HelpBox("MasterBuildProfilesに空のビルドプロファイルがあります。", MessageType.Warning); }
            if (AutoBuilderSettings.HasDuplicateBuildProfiles(AutoBuilderSettings.instance.MasterBuildProfiles)) { EditorGUILayout.HelpBox("MasterBuildProfilesに重複するビルドプロファイルがあります。", MessageType.Warning); }
            ...
            if (AutoBuilderSettings.HasEmptyBuildProfile(AutoBuilderSettings.instance.DevelopBuildProfiles)) { EditorGUILayout.HelpBox("DevelopBuildProfilesに空のビルドプロファイルがあります。", MessageType.Warning); }
            if (AutoBuilderSettings.HasDuplicateBuildProfiles(AutoBuilderSettings.instance.DevelopBuildProfiles)) { EditorGUILayout.HelpBox("DevelopBuildProfilesに重複するビルドプロファイルがあります。", MessageType.Warning); }
