...
- 以下の W-03（背景トグル）・W-09（ID）・W-11（数値の表）は、既存の書き方を崩す場合は使わない。ツールが参照するページ（コード規定・設計思想・QA など）と、数値が多い箇所に限って使う。
- 言い回しを整えるだけの書き換えはしない。
...
| W-02 | **ページの本文に「このページについて」の節を作らない。** 対象読者は書かない。最終確認日は Notion の最終更新日で分かるので書かない。どのページにも必ず入れる情報（何の正本か、状態など）は、本文ではなく **DB のプロパティ（カラム）** にする（§5.5 の案） | 本文を読まなくても、DB の一覧で状態と正本の範囲が分かるようにするため（2026-09-22 八幡さんの決定） |
...
| W-18 | 仕様リストのページは「仕様概要（関連ページ・関連用語）→ 詳細説明 → 仕様意図」の順にする。**仕様意図はページの末尾**に置き、その中に「経緯」（日付・提案・検討・決定・変更・却下と判断の理由・出典）と「却下・保留した案」を網羅する。仕様を変えたり、提案を却下したりしたときは、ここに追記する | 同じ提案が再び出たときに、過去の検討と判断の理由をすぐ参照できるようにするため（2026-09-22 八幡さんの決定）。仕様の本文を先に読めるよう、長くなる経緯は末尾に置く |

### 5.2 使うブロック

| # | 規約 | 理由 |
| --- | --- | --- |
| W-05 | 使ってよいブロックは、見出し・段落・箇条書き・番号付きリスト・表（シンプルテーブル）・コード・引用・コールアウトとする | Markdown に変換しても意味が崩れないため。**必要に応じて他のブロックの使用も検討する**（2026-09-22 八幡さん。提案は §5.6） |
...
| 適用範囲 | マルチセレクト | 製品版 / 体験版（TGS 2026） | 体験版だけの仕様を区別する（W-04） |
| 値の正本 | セレクト | このページ / マスターデータ / コード | 数値をどこで管理するかを示す（決定 No.14。ステータスはマスターデータ） |
| キャッシュ対象 | チェックボックス | ✓ | ツール用キャッシュ（`SinfoniaOperator/notion-cache-map.json`）に載っているページ（W-14） |
...
| リンクドデータベースビュー | DB の一覧を別ページで見せるとき（本文の代わりにはしない） | W-06 のとおり本文は入れないが、一覧の表示には使ってよい |
