...
- 決定（2026-09-22 八幡）: R14 敵の攻撃ダメージはマスターデータに移し、敵の種類・攻撃ごとに設定する。🏗️ クラスの `EnemyAttackUsecase` の行に書いた
- 決定（2026-09-22 八幡）: R17 発見状態のシステムを無効にし、常に発見状態に固定する（モジュールは再利用できる形で残す）。🏗️ クラスの Condition ノードの行と「④ View」に書いた
- 決定（2026-09-22 八幡）: R19 キル数の計上とウェーブ全滅判定のタイミングを揃える。「③ Adaptor」の全滅判定の文に書いた
- 決定（2026-09-22 八幡）: R15 マーク付き砲兵は専用のプレハブを作る。🏗️ クラスの `EnemyInfantrySpawner` / `EnemyArtillerySpawner` / `EnemyPools` の行に、現状（通常の砲兵と同じプレハブ）と未実装を書いた
- 決定（2026-09-22 八幡）: R16 歩兵の攻撃判定の太さと 7m 未満での後退は見直す。🏗️ クラスの `EnemyRaycastDetectView` の行に現状（レイ判定で太さなし）を書き、【要確認: 見直し後の値】にした
- 決定（2026-09-22 八幡）: R18 攻撃後の行動は、射程外なら接近・障害物なら迂回を優先し、それ以外は重み付きの抽選とする。🏗️ クラスの `EnemyPostAttackBehaviorUsecase` の行に仕様と未実装（#2072）を書いた
- 決定（2026-09-22 八幡）: R34 ダメージ数値は、Just で色が変わり、クリティカルは数字の背景に専用の絵、キルコードは変わらない（「スキル」種別は仕様から外す）。🏗️ クラスの `DamageNumberDTO` / `DamageNumberType` の行に書いた（#2086）
- 実装の修正が必要: R18 射程外の接近と障害物の迂回を抽選より優先させる（Issue #2072、根拠 `Assets/Scripts/Runtime/2.Application/InGame/Enemy/EnemyPostAttackBehaviorUsecase.cs` の `ChooseKind`）
- 実装の修正が必要: R34 使われていない `DamageNumberType.Skill` を外す（Issue #2086、根拠 `Assets/Scripts/Runtime/3.Adaptor/InGame/Enemy/DamageNumberType.cs`、`EnemyHealthHudPresenter.cs:105-115` は `JustHit` と `Normal` だけを返す）
- 実装の修正が必要: R14 敵の攻撃ダメージの固定値 10 をマスターデータへ移す（Issue #2004、根拠 `Assets/Scripts/Runtime/2.Application/InGame/Enemy/EnemyAttackUsecase.cs:50`）
- 実装の修正が必要: R17 発見システムを無効にして常に発見状態にする（Issue #2051、根拠 `Assets/Scripts/Runtime/4.View/InGame/Enemy/BehaviorGraphNode/Action/WaitForDiscoveryAction.cs`、`Assets/Scripts/Runtime/4.View/InGame/Enemy/AIFacade/EnemyStateFacade.cs:42,106-113`）
- 実装の修正が必要: R19 キル数の計上（HP0 の瞬間）と全滅判定（死亡演出の完了後）を揃える（Issue #2052、根拠 `Assets/Scripts/Runtime/6.Composition/InGame/Enemy/EnemyLifeCycle.cs:773-791`）
- 実装の修正が必要: R15 マーク付き砲兵の専用プレハブを作る（Issue #2049、根拠 `Assets/Level/Data/Master/InGame/Enemy/Definitions/EnemyDefinition_Artillery_Marked.asset:19` と `EnemyDefinition_Artillery_Default.asset:19` の `_viewPrefab` が同じ GUID）
- 実装の修正が必要: R16 歩兵の攻撃判定の太さと 7m 未満での後退を見直す（Issue #2050、根拠 `Assets/Scripts/Runtime/4.View/InGame/Enemy/EnemyRaycastDetectView.cs:147,181` のレイ判定）
...
- 要確認:
  - R16: 歩兵の攻撃判定の太さと、7m 未満での後退の見直し後の値（企画。Issue #2050）
  - R19: キル数の計上と全滅判定を、HP0 の瞬間と死亡演出の完了後のどちらに揃えるか（企画）
...
| **`EnemyAttackUsecase`** / **`EnemyAttackReservationUsecase`** | Application | 攻撃の実行と、ビートに同期した攻撃予約。攻撃のダメージは、マスターデータで敵の種類・攻撃ごとに設定する。現状は`EnemyAttackUsecase`の固定値10で、マスターデータへ移す必要がある（Issue #2004） |
...
| **`EnemyPostAttackBehaviorUsecase`** | Application | 攻撃後の行動（再攻撃/味方合流/障害物接近）を重み抽選で決定し、合流・接近の場合は上書き移動先を算出する。仕様は、射程外なら接近・障害物なら迂回を優先し、それ以外で抽選する（再攻撃0.5 / 味方に合流0.25 / 障害物に近づく0.25）。優先の判定は未実装（#2072） |
...
| **`DamageNumberDTO`** / **`DamageNumberType`** / **`IDamageNumber`** | Adaptor | 被ダメージ数値表示の通知。表示の種類は、Justで色が変わり（`JustHit`）、クリティカルで数字の背景に専用の絵が付く。キルコードによるダメージは通常と同じ表示で、列挙の`Skill`は仕様から外した（どこからも使われていない。#2086） |
...
| **`IsTargetInAttackRangeCondition`** / **`IsAimSightClearCondition`** / **`IsAttackingCondition`** / **`IsStunnedCondition`** / **`IsDiscoveredCondition`** / **`IsBattleAiActivatedCondition`** | View | BehaviorGraphのConditionノード。`IsDiscoveredCondition`は発見済みかどうかを（敵は常に発見状態に固定する。下記「④ View」）、`IsBattleAiActivatedCondition`は戦闘AIが有効かどうか（Missionの進入時アクションで切り替わる）を分岐条件として提供する |
...
| **`EnemyRaycastDetectView`** / **`TripleShotRaycastDetectView`** / **`NearestAttackPositionSearchView`** | View | 索敵と攻撃位置探索のUnity側実装。`EnemyRaycastDetectView`は攻撃予兆のデカール（2拍前に追従表示、1拍前に方向固定）も表示する。歩兵の攻撃判定はレイで、太さが無い。攻撃判定の太さと、7m未満で離れる動きは見直す（Issue #2050。見直し後の値は【要確認: 企画に】） |
...
| **`EnemyInfantrySpawner`** / **`EnemyArtillerySpawner`** / **`EnemyPools`** | Composition | 歩兵・砲兵の生成と、プレハブのプール管理。入場に失敗したら位置を再抽選して再試行する。`EnemyPools.PrewarmEnemy`はロード中に全ウェーブの敵を事前生成し、画面外カメラで1回描画してシェーダーをウォームアップする。マーク付き砲兵は、通常の砲兵と同じプレハブ（敵定義`EnemyDefinition_Artillery_Marked`の`_viewPrefab`）を使っている。専用のプレハブを作る（未実装。Issue #2049） |
...
敵は常に発見状態とする。発見状態のシステム（`WaitForDiscoveryAction`・`IsDiscoveredCondition`・`EnemyStateFacade`の`IsPlayerDiscoverable`/`LookAround`/`Discover`）は無効にし、モジュールは再利用できる形で残す。現状は発見システムが有効で、未発見の敵は被弾や味方の撃破では発見状態にならない（未対応。Issue #2051）。
