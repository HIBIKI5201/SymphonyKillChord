...
7. **ロックオンは実装の不具合（決定 2026-09-22 八幡）。** 仕様（`仕様概要 / カメラワーク`）が正しく、実装が誤りと決まった。ターゲットシステム・② ターゲット選択フロー・② ロックオン開始フロー・③ オートロックオン解除フローの本文を仕様どおりに直した。
   ...
   - 対象の消失: マニュアルは仕様どおり次の敵へ再ロックする（実装は解除する。`CameraSystemView.cs:643-647`）。オートは仕様に定義が無く【要確認】。左右切替・オート中のロックオン入力での解除も仕様に定義が無く【要確認】。
...
| 3bf7c2c6-cc02-811a-bbad-f6faee96225f | ② ロックオン開始フロー（入力・被弾時） | 可 | 一部古い | 本文置き換え | 3bf7c2c6-cc02-811a-bbad-f6faee96225f.md | 「攻撃者 ID」を DefenderId に訂正し、分岐を 4 つに整理。選定の書き方を仕様に戻した（決定 2026-09-22）。CM-01 / CM-05 |
| 3bf7c2c6-cc02-8169-8fe2-d1bfb8f182c0 | ③ オートロックオン解除フロー | 可 | 一部古い | 本文置き換え | 3bf7c2c6-cc02-8169-8fe2-d1bfb8f182c0.md | 解除条件に現在の値を併記。マニュアルの対象消失は仕様どおり再ロック（決定 2026-09-22、実装の不具合）。移動での解除（CM-02）は決定の範囲外で現状＋【要確認】。仕様に無い 2 条件とオートの対象消失も【要確認】。CM-02 / CM-04。→ 決定（2026-09-22 八幡）No.18: 解除条件は実装どおりに確定し【要確認】3 件を外した。強い視点移動での切り替えを未実装として追加 |
| 3957c2c6-cc02-80cd-94c0-f412b11ffcf7 | 敵 | 可 | 一部古い | 本文置き換え（子ページ⑤を新設） | 3957c2c6-cc02-80cd-94c0-f412b11ffcf7.md | TGS 期に追加された実装、Mission との双方向の依存、ステージ概念の所在を追記。EN-25 / EN-30。→ 決定（2026-09-22 八幡）No.2 / No.13 / No.16: 説明語をウェーブに統一。硬直は実装の不具合（削除が必要）。射程外での取り消しは現状と本来の仕様を併記 |
| 3bf7c2c6-cc02-8193-b6cf-cfd3ff3cb46f | ① AI 移動制御フロー（毎フレーム） | 可 | 一部古い | 本文置き換え | 3bf7c2c6-cc02-8193-b6cf-cfd3ff3cb46f.md | 存在しない EnemyView を EnemyMoveView に修正。攻撃の予約は BehaviorGraph 側で行う。→ 決定（2026-09-22 八幡）No.16: 射程外での取り消しに現状と本来の仕様（予兆の後は攻撃を実行）を併記 |
...
| 3bf7c2c6-cc02-818e-9975-fa89910dbb43 | ③ ウェーブスポナー & Wave開始通知フロー（ウェーブクリア／開始時） | 可 | 一部古い | 本文置き換え | 3bf7c2c6-cc02-818e-9975-fa89910dbb43.md | 時間切れ・入場待ち・OnWaveAllCleared を追加。EN-17。→ 決定（2026-09-22 八幡）No.2: 説明語をウェーブに統一 |
...
| 39e7c2c6-cc02-80d5-ac4e-cd46b87fd375 | ミッション | 可 | 一部古い | 本文置き換え | 39e7c2c6-cc02-80d5-ac4e-cd46b87fd375.md | 会話・全ウェーブ撃破・チュートリアル表示を追加。Container の中身・型名・依存を修正。EN-24 / SC-09 / TU-03。→ 決定（2026-09-22 八幡）No.2 / No.20 / No.21: 説明語をウェーブに統一。コンボは 3 以上で表示（実装は 4）、キルコード発動ではリセットしない（実装はリセットする） |
...
| 3bf7c2c6-cc02-8144-aeef-c67ace0b4a39 | ② 敵撃破→クリア判定→保存→リザルト表示フロー | 可 | 一部古い | 本文置き換え | 3bf7c2c6-cc02-8144-aeef-c67ace0b4a39.md | HP0 での即時通知と、全ウェーブ撃破の 2 経路を記載。EN-16。→ 決定（2026-09-22 八幡）No.2: 説明語をウェーブに統一 |
...
| 39e7c2c6-cc02-80e9-933f-f02210602bf6 | ターゲットシステム | 可 | 一部古い | 本文置き換え | 39e7c2c6-cc02-80e9-933f-f02210602bf6.md | 距離のクエリ 2 種・候補・EOnLockOnAcquired を追加し、依存元を修正。選定を仕様（視界範囲内・HP 優先）に戻した（決定 2026-09-22、実装の不具合）。CM-01 / CM-16 / CM-17 |
