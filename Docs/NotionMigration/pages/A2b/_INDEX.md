...
7. **ロックオンは実装の不具合（決定 2026-09-22 八幡）。** 仕様（`仕様概要 / カメラワーク`）が正しく、実装が誤りと決まった。ターゲットシステム・② ターゲット選択フロー・② ロックオン開始フロー・③ オートロックオン解除フローの本文を仕様どおりに直した。
   ...
   - オートの解除: 移動操作での解除（CM-02）は決定の範囲外。実装は移動入力で解除しない（`CameraSystemView.cs:727-742`）ため、現状を書いて【要確認: 移動入力で解除する仕様か。実装のミスとして扱うか】とした。仕様に無い「3 秒命中なし」「画面外」の 2 条件も【要確認: 実装のミスとして扱うか】。
...
| 3bf7c2c6-cc02-8169-8fe2-d1bfb8f182c0 | ③ オートロックオン解除フロー | 可 | 一部古い | 本文置き換え | 3bf7c2c6-cc02-8169-8fe2-d1bfb8f182c0.md | 解除条件に現在の値を併記。マニュアルの対象消失は仕様どおり再ロック（決定 2026-09-22、実装の不具合）。移動での解除（CM-02）は決定の範囲外で現状＋【要確認】。仕様に無い 2 条件とオートの対象消失も【要確認】。CM-02 / CM-04 |
