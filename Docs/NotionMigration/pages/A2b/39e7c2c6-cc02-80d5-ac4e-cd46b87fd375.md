...
- 整合性:
  ...
  - Master と Demo でチュートリアルのミッション定義が別アセットになっていて、会話の配置が違う（TU-04）。→ 決定 R63: Demo が正本で、Master を合わせる
- 変更点:
...
- 決定（2026-09-22 八幡）: R22 サブミッションの数は現状のまま（各 3 個）。「⑤ Infrastructure」に実データの数の表を足した。`Test`（ステージ 05〜09・ボス）は 2 個なので【要確認】を付けた（根拠 `Assets/Level/Data/Master/OutGame/StageSelect/Mission/MissionDefinition_1.asset:31`、`MissionDefinition_2.asset:31`、`MissionDefinition_3.asset:30`、`MissionDefinition_Tutorial.asset:119`、`Stages/BattleStage_*.asset:29`）
- 決定（2026-09-22 八幡）: R20 ステージ 05〜09・ボスの仮データは Issue にする。「⑤ Infrastructure」に、仮のミッション定義 `Test` を使っていることと Issue #2053 を書いた
- 決定（2026-09-22 八幡）: R63 チュートリアルの定義は Demo が正本で、Master を合わせる。「⑤ Infrastructure」の `MissionDefinition_Tutorial.asset` の文を「Demo 側を正本とし、Master 側を合わせる（未実施、#2075）」にし、【要確認】（TU-04）を外した
- 実装の修正が必要: R63 Master の `MissionDefinition_Tutorial.asset` を Demo に合わせる（Issue #2075、根拠 `Assets/Level/Data/Master/OutGame/StageSelect/Mission/` と `Assets/Level/Data/Demo/OutGame/StageSelect/Mission/` の同名アセットで会話の配置が違う）
...
- Discord 照合: 照合済み（2026-09-22、進捗共有「クライアント部門」「バトルチュートリアル準備」「InGameシーン」「コンボUI」、#プログラマー）。PR #1550 は 2026-09-14 に提出・承認され、会話を MissionStep から外すかの検討は無い。サブミッションは 2026-03-15 に企画の要望で「同じ種類の条件を重ねて設定できる（例: 2 分以内と 1 分以内）」ことになった（進捗共有「InGameシーン」）。コンボは被弾と空振りで途切れる実装で始まった（2026-08-09、進捗共有「コンボUI」）。スキル 5 の HP 消費とコンボの関係の検討は無い。2026-09-22 の決定との食い違いは無い。本文の変更は無い
- 要確認:
  - R22: ミッション定義 `Test`（ステージ 05〜09・ボス）のサブミッションが 2 個である。3 個に揃えるか（企画）
  ...
  - PR #1550 の残論点「会話を MissionStep から外した方が良いかもしれない」の結論（プログラム担当。Discord では PR の提出・承認・マージの連絡だけで、この論点の検討は無い）
...
`MissionDefinitionAsset`が`[SerializeReference, SubclassSelector]`でクリア/失敗/評価条件をインスペクタから多態的に構成できるようにし、`Create()`でDomainオブジェクトへ変換する。`MissionDefinitionRepository`と`EnemyMissionKeyRepository`がそれぞれの定義をIDで引く。両リポジトリはAddressablesのアドレス`MissionDefinitionRepository`/`EnemyMissionKeyRepository`でロードし、ReleaseビルドではMaster側（`Assets/Level/Data/Master/OutGame/StageSelect/Mission/`）、DemoビルドではDemo側（`Assets/Level/Data/Demo/OutGame/StageSelect/Mission/`）が使われる。チュートリアルのミッション定義（`MissionDefinition_Tutorial.asset`）はMasterとDemoで別アセットであり、会話の配置が違う。Demo側を正本とし、Master側をDemoに合わせる（未実施、#2075）。

現在のミッション定義と、サブミッション（評価条件）の数は次のとおりである（Master・Demo とも同じ）。ステージ05〜09とボスは、仮のミッション定義`Test`を使っている。仮データを差し替える作業は Issue #2053 で扱う。

| ミッション定義 | ID | サブミッションの数 | 使っているステージ |
| --- | --- | --- | --- |
| `MissionDefinition_1` | `AllKill` | 3 | ステージ01・03・04 |
| `MissionDefinition_2` | `Test` | 2 | `BattleStage`、ステージ05〜09、ボス |
| `MissionDefinition_3` | `Durability` | 3 | ステージ02 |
| `MissionDefinition_Tutorial` | `Tutorial` | 0 | チュートリアル |

サブミッションは各ステージ3個である。【要確認: `Test`（ステージ05〜09・ボス）は2個である。3個に揃えるかを企画に】
