...
目標ステップの達成条件とは別に、そのステップへ入った瞬間に一度だけ起こす副作用を`ObjectiveSequenceStepAsset`へ設定できる。デコレータ条件による副作用（ポップアップ・ウェーブ・バフ）と違い、条件の判定には関与しない。実行器は`InGameMissionInitializer`のReadyフェーズで登録する。会話の実行器（`MissionDialogueController`）は、ミッションのどこかのステップに会話がある場合だけ登録される。対応する実行器が無いアクションは実行されず、警告ログが出る。
