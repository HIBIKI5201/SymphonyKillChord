# ③ ウェーブスポナー & Wave開始通知フロー（ウェーブクリア／開始時）

- page_id: 3bf7c2c6-cc02-818e-9975-fa89910dbb43
- Notion パス: Symphony Kill Chord / システム概要 / 敵 / ③ ウェーブスポナー & Wave開始通知フロー（ウェーブクリア／開始時）
- スナップショットの最終更新: 2026-08-17T17:28:15.993Z
- 書き込み許可: 可（システム概要 配下）
- 処置: 本文置き換え
- 確認日 / コミット: 2026-09-22 / origin/develop 73fefeb45

## 判定

- 信頼性: 一部古い — 全滅時の分岐しか描かれていない。実装は「全滅」「`WaveDuration` の経過」「ミッションの目標ステップ」の 3 つの起点で次の Wave を生成し、最初の Wave はゲーム開始時に生成する（`Assets/Scripts/Runtime/4.View/InGame/Enemy/EnemyWaveTimerView.cs`、`3.Adaptor/InGame/Mission/MissionWaveController.cs:78-90`）。生成前の入場待ち数の予約（PR #1721）と、最終 Wave の全滅通知 `OnWaveAllCleared` も無い。スポナーの参加者 `EnemyInfantrySpawner / EnemyArtillerySpawner` は、実際には `IEnemySpawner`（`EnemySpawnerRouter`）を経由する
- 整合性: 仕様概要 / ステージルール / フェーズについて 3497c2c6-cc02-802d-9439-de4cfc3001ab（A1 担当、EN-17）の「殲滅 / 一定時間経過」と一致させた。親ページ 3957c2c6-cc02-80cd-94c0-f412b11ffcf7 の新規子ページ ⑤（死亡処理）と、全滅判定が死亡演出の後になる点を揃えた
- 変更点:
  - 冒頭の説明文: 旧: 「ウェーブ内の全敵が撃破されると次ウェーブが生成され」だけ → 新: 3 つの起点、入場待ち数、ループ、ミッションが Wave 開始を制御する場合を追記した
  - 図: 開始の起点（ゲーム開始・全滅・時間切れ・目標ステップ）の分岐を追加した。`ReserveEnemySpawns`、`SetLastWave`、入場成功時の `AddEnemyCount` を追加した。スポナーを `EnemySpawnerRouter` 経由に直した
- 決定（2026-09-22 八幡）: No.2 敵の出現単位の説明語「Wave」を「ウェーブ」に直した（冒頭の説明文と図の注記）。クラス名・メンバー名・ページ名は変えていない
- 織り込んだ反映項目: EN-17（③フローへの時間切れ分岐）, EN-16（全滅判定のタイミング）, EN-25（`ReserveEnemySpawns`）
- 出典: 実装（`3.Adaptor/InGame/Enemy/EnemyWaveSpawnerController.cs`、`EnemyWaveSpawnerState.cs:61-79`、`4.View/InGame/Enemy/EnemyWaveTimerView.cs`、`1.Domain/InGame/Enemy/EnemyWaves.cs`、`3.Adaptor/InGame/Mission/MissionWaveController.cs`、`6.Composition/InGame/Enemy/EnemyInitializer.cs:193-217`、`6.Composition/InGame/Stage/StageEffectInitializer.cs`）、PR #1721
- 決定（2026-09-22 八幡）: R19 キル数の計上とウェーブ全滅判定のタイミングを揃える。冒頭の説明文の 2 段落目に、現状の時間差と「同じタイミングで行うように直す」を書いた
- 実装の修正が必要: キル数の計上（HP0 の瞬間）と全滅判定（死亡演出の完了後）を同じタイミングにする（Issue #2052、根拠 `Assets/Scripts/Runtime/6.Composition/InGame/Enemy/EnemyLifeCycle.cs:773-791`）
- 要確認: キル数の計上と全滅判定をどちらのタイミングに揃えるか（企画）。同時に存在できる敵数に上限が無いことを許容するかは仕様概要側の要確認

## 適用する本文

次のウェーブは、ウェーブ内の全敵の撃破、`WaveDuration`（秒）の経過、ミッションの目標ステップのいずれかを起点に生成され、ウェーブ開始が他モジュールへ通知される。最初のウェーブはゲームプレイ開始時（`EnemyWaveTimerView.StartGameplay`）に生成する。時間切れで進んだ場合、前のウェーブの残りと重なって出現する。ウェーブ列にループが設定されていれば、最後まで進んだあとループ開始位置へ戻る。

生成を始める前に、そのウェーブの出現予定数を「入場待ち数」として予約し、敵が入場を終えるたびに生存数へ移す。全滅は生存数と入場待ち数がともに0になったときだけ成立する。生存数が減るのは敵の死亡演出が終わってからである（⑤）。ミッションのキル数はHPが0になった瞬間に計上されるため、現状は全滅判定との間に時間差がある。キル数の計上と全滅判定は同じタイミングで行うように直す（Issue #2052）。どちらに揃えるかは【要確認: HP0 の瞬間と死亡演出の完了後のどちらに揃えるかを企画に】。

ミッションの目標シーケンスに`WaveStartClearCondition`を持つステップがある場合は、全滅と時間切れによる自動生成を止め、`MissionWaveController`が該当ステップの開始時に次のウェーブを生成する。

```mermaid
sequenceDiagram
    autonumber
    participant EWTimerView as EnemyWaveTimerView
    participant MWave as MissionWaveController (Missionモジュール)
    participant EWState as EnemyWaveSpawnerState
    participant EWCont as EnemyWaveSpawnerController
    participant Spawner as EnemySpawnerRouter (IEnemySpawner)
    participant StageEffect as StageEffectInitializer (Stageモジュール)

    alt ゲームプレイ開始
        EWTimerView ->> EWCont: SpawnNextWave（Missionが制御する場合は呼ばない）
    else 全敵が撃破された（OnWaveCleared）
        EWState -->> EWCont: OnWaveCleared イベントコールバック (SpawnNextWave)（Missionが制御する場合は購読しない）
    else WaveDuration が経過した
        EWTimerView ->> EWCont: SpawnNextWave（Missionが制御する場合はタイマーを止めるだけ）
    else WaveStartClearCondition を持つ目標ステップが始まった
        MWave ->> EWCont: SpawnNextWave
    end
    EWCont ->> EWCont: 次の EnemyWaveDefinition を取得
    alt 次ウェーブが存在する
        EWCont ->> EWState: ReserveEnemySpawns（出現予定数を入場待ち数として予約）
        EWCont ->> EWState: SetLastWave（最終ウェーブの場合）
        EWCont ->> EWState: NotifyWaveStarted(waveIndex, definition)
        EWState -->> StageEffect: OnWaveStarted イベント発火（StageEffectIdsを含む）
        StageEffect ->> StageEffect: IDから自身のカタログを検索し、該当演出をBGM同期で再生
        EWCont ->> Spawner: 敵定義ID・数・スポーン候補を指定して生成指示（歩兵/砲兵のスポナーへ振り分け）
        Spawner -->> EWState: 入場に成功した敵ごとに AddEnemyCount（入場待ち数 → 生存数）
        EWCont ->> EWTimerView: ウェーブタイマー設定 (SetTimer: waveDuration)
    else 全ウェーブクリア
        EWCont ->> EWTimerView: タイマー停止 (StopTimer)
    end
    Note over EWState: 最終ウェーブで生存数・入場待ち数が0になると OnWaveAllCleared を発火し、MissionWaveController がミッションへ全ウェーブ撃破を伝える
```
