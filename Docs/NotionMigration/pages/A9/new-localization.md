...
- 変更点:
  ...
  - 決定 R54（2026-09-22 八幡）: 「仕様概要」に「ゲーム内のすべての表示を日本語と英語に対応させる」を追記。「### 訳語の決め方」: 旧「英訳は、外部のWorkbookで確定した訳を使う【要確認: …】」→ 新「訳語の正本は Google Drive にある CSV」。「### ローカライズしていない表示」: 冒頭に「すべての表示を日英に対応させる方針で、次の表示は未対応（#2067）」と書き、末尾の【要確認: 今後ローカライズするか…】を外した
  - 決定 R55（2026-09-22 八幡）: 「### 訳語の決め方」に「画面の用語は『未解放』『作戦』『Settings』に揃える（#2028）」を追記（今の `ui.skill.locked`「未開放」、環境設定の英訳「Display Settings」を直す）
...
- 要確認:
  - 訳語の正本の CSV（Google Drive、決定 R54）の管理者（要企画確認、OG-214）。置き場所は決定 R54 で解決
...
## 仕様概要
ゲーム内の表示を、プレイヤーが選んだ言語で表示する。対応している言語は日本語と英語の2つである。ゲーム内のすべての表示を、日本語と英語に対応させる。
...
### 訳語の決め方
- 訳語の正本は、Google DriveにあるCSVである。英訳はこのCSVで確定した訳を使う【要確認: CSVの管理者を企画に確認】
- 用語の英名は、用語リストの「英名」に合わせる
- 画面の用語は「未解放」「作戦」「Settings」に揃える（#2028。今は`ui.skill.locked`が「未開放」、環境設定の英訳が「Display Settings」になっている）
...
### ローカライズしていない表示
すべての表示を日英に対応させる方針だが、次の表示は未対応（#2067）である。翻訳テーブルを通らないため、言語を切り替えても変わらない。
- 日本語のまま表示されるもの
  - ロード画面のTips
