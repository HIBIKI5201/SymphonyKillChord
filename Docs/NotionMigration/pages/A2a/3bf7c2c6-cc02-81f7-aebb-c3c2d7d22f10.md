...
| **表示閾値** | `InGameMissionInitializer`の`_comboVisibleCount`（現在値 4。`InGame.unity`で設定） |
...
`ComboHudViewModel`が表示内容を保持し、`ComboHudView`が画面へ表示する。<br>`ComboHudView`は、コンボ数が表示閾値（現在値 4）未満のとき表示ルートを非表示にしてテキストを空にする。閾値以上のときは数字を更新し、0.1 秒の揺れ演出（LitMotion の Punch）を再生する。
