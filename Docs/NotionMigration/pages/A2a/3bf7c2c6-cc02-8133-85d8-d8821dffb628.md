...
- 決定（2026-09-22 八幡）: R13 入力履歴は先行入力のために残す。冒頭の説明文に「先行入力のために残している」と書き、残すか削除するかの【要確認】を外した
- 要確認: なし

## 適用する本文

入力を時刻付きでバッファへ残す。記録するのはオプション・決定・キャンセル・回避・攻撃・移動・マウス視点の入力である。記録した履歴は先行入力のために残している。記録した履歴を読むゲーム側の処理は現在無い。
