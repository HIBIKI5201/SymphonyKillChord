...
- 決定（2026-09-22 八幡）: R12 ロコモーションの BlendTree 化と音楽の位相への同期は続ける。「🎵 ロコモーションの音楽同期」節の最後の段落を「続ける（未実装）」にし、継続するかの【要確認】を外した
- 実装の修正が必要: R12 ロコモーションを常駐 BlendTree へ移し、途中生成の敵も曲の現在時刻に合わせて頭出しする（Issue #2048、根拠 `Assets/Scripts/Runtime/4.View/InGame/Animation/PlayableAnimationController.cs:45-55` は生成時に `Play()` するだけ）
...
ロコモーションを`AnimatorControllerPlayable`の常駐BlendTreeへ移す計画（BlendTreeは一度も遷移しない。BlendTreeに入れるクリップは同じ拍数で制作する）は続ける（未実装）。ロコモーションの位相を音楽に同期させる要件も続ける。曲の途中で生成された敵の位相のずれは、この計画の中で直す。
