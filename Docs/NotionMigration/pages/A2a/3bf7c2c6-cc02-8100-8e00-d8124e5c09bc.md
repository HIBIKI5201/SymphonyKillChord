...
- 要確認:
  - 【要確認: 企画】UI とアウトゲームの決定が Gamepad East（Xbox の B / PS の ○）、キャンセルが South（Xbox の A / PS の ×）の日本式配置である。英語圏向けにも維持するか（OG-218）
...
## 🏗️ クラス {toggle="true"}
	...
	| **`InputBufferingQueue`** | Domain | 入力をバッファリングする。現在はゲーム側に読み手が無い |
...
ゲームパッドの East は Xbox の B / PlayStation の ○、South は Xbox の A / PlayStation の × である。決定を East、キャンセルを South に置く日本式の配置である。【要確認: 英語圏向けにもこの配置を維持するか、企画に確認】
