# ① BGM 再生とリズム同期の更新フロー（毎フレーム）

- page_id: 3bf7c2c6-cc02-81fa-a314-c728299d6ede
- Notion パス: Symphony Kill Chord / システム概要 / 音楽 / ① BGM 再生とリズム同期の更新フロー（毎フレーム）
- スナップショットの最終更新: 2026-08-17T13:21:06.466Z
- 書き込み許可: 可
- 処置: 本文置き換え

## 判定

- 信頼性: 一部古い — 大筋（`MusicSyncView` → `MusicPlayer.Time` → `MusicSyncController.Tick` → `MusicSyncState.UpdatePlayTime` / `IMusicSyncService.Update`）は実装どおり。ただし、判定オフセットの加算（PR #1675）、ゲームプレイ中だけ回す制御（`SetGameplayActive`）、巻き戻し検知とリズムタイムアウト（PR #1893・#1896）が無い。本文の「AudioSource の再生タイム」は誤りで、`MusicPlayer` は CRI の `CriAtomSource` を使う（`Runtime/4.View/Persistent/Music/MusicPlayer.cs:1,12,19`）
- 整合性: 親ページ `音楽`（3957c2c6-cc02-806b-9cc8-fbe9c9e8bf63）の原稿と一致させた
- 変更点:
  - 冒頭の説明文: 旧: AudioSource の再生タイムを元に → 新: `MusicPlayer`（CRI）の再生時間を元に、ゲームプレイ中だけ
  - シーケンス図: 実行条件、再生時間が負のときの `ResetPlayback`、判定オフセットの加算、`Update` 内の巻き戻し検知・リズムタイムアウト・予約実行の順を追加
- 織り込んだ反映項目: BT-09（`OnRhythmTimedOut`・`ResetPlayback`・判定オフセットの注入）
- 出典: 実装 `Runtime/4.View/InGame/Music/MusicSyncView.cs`（`Update`、`SetGameplayActive`）、`Runtime/3.Adaptor/InGame/Music/MusicSyncController.cs`（`Tick`）、`Runtime/2.Application/InGame/Music/MusicSyncService.cs:36-93`、`Runtime/1.Domain/InGame/Music/RhythmJudgmentDefinition.cs:29-32`
- 決定（2026-09-22 八幡）: No.11 リズム判定オフセットは判定と表示の両方をずらす（範囲±0.30 秒・0.05 秒刻み）。冒頭の説明文の後ろに、オフセットを再生時間に加えて `MusicSyncState`（表示）と `IMusicSyncService`（判定）の両方へ渡す段落を追加した（`Assets/Scripts/Runtime/3.Adaptor/InGame/Music/MusicSyncController.cs:36-41`、`Assets/Scripts/Runtime/1.Domain/Persistent/Savedata/EnvironmentSettingsData.cs:69-75`）
- 要確認:
  - 【要実機確認】判定が直前の入力との間隔で行われるため、オフセットがプレイヤー自身の入力の判定に効きにくい可能性がある（No.11）

## 適用する本文

`MusicPlayer`（CRI）の再生時間を元に、ゲームプレイ中だけ毎フレーム BPM・拍情報が更新される処理フローである。ゲームプレイの開始・終了は`MusicSyncInitializer`の`StartGameplay`/`StopGameplay`が`MusicSyncView.SetGameplayActive`で切り替え、終了時とBGMのCue変更時には同期状態（入力履歴・予約・ゲージ基準）を破棄する。

リズム判定オフセット（環境設定。範囲±0.30秒、0.05秒刻み）は再生時間そのものに加える。オフセットを加えた再生時間を`MusicSyncState`（ガイドなどの表示）と`IMusicSyncService`（判定）の両方へ渡すため、判定と表示の両方が同じだけずれる。ただし判定は直前の入力からの経過で行うため、プレイヤー自身の入力の判定にはオフセットが効きにくい可能性がある。【要実機確認: オフセットを変えたときに攻撃の判定がずれるかを実機で確かめる】

```mermaid
sequenceDiagram
    autonumber
    participant MSView as MusicSyncView
    participant MPlayer as MusicPlayer
    participant MSCont as MusicSyncController
    participant Env as IEnvironmentSettingsViewModel
    participant MSState as MusicSyncState
    participant MSService as IMusicSyncService

    Note over MSView: 毎フレームの Update ループ（ゲームプレイ中・Cue 設定済み・BPM 設定済みのときだけ）
    MSView ->> MPlayer: 現在の再生時間を取得 (MusicPlayer.Time)
    MPlayer -->> MSView: playTime (double) を返却
    alt playTime が負
        MSView ->> MSCont: ResetPlayback()
    else それ以外
        MSView ->> MSCont: タイミング更新 (Tick: playTime)
        MSCont ->> Env: リズム判定オフセットを取得 (RhythmOffsetSeconds)
        Env -->> MSCont: オフセット秒（未取得なら 0）
        MSCont ->> MSState: 再生時間を反映 (UpdatePlayTime: playTime + オフセット)
        MSCont ->> MSService: ビート進行の更新 (Update: playTime + オフセット)
        MSService ->> MSService: 巻き戻しを検知したら ResetPlayback
        opt 最後の入力から2小節経過
            MSService ->> MSService: 入力履歴を破棄し OnRhythmTimedOut を通知
        end
        MSService ->> MSService: 実行時刻に達した予約済みアクションを実行
    end
```
