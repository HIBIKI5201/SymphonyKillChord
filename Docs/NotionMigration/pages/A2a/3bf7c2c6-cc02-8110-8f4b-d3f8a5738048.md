...
入力途中の状態を、リズムガイド下のスキル入力ガイド・クロスヘア・装備スキル一覧の3か所へ反映する。進捗が変わるのは、拍が入力されたとき・スキルが発動したとき・進捗がリセットされたとき（リズムタイムアウトなど）である。

- スキル入力ガイド: 未入力のときは全スキルの走り出しを、入力中は最も進んでいるスキルだけを表示する。各スキルの「次に入力する1拍」のアイコンを、リズムガイドの同じ拍のジャスト位置へ左右対称に出す。その拍のジャスト中心を含む同じ色のブロックの区間に色帯を付ける（色帯はスキルの次の対象拍にだけ付け、ゲージ全体には出さない）。クールダウン中はゲージで残り時間を示す
- クロスヘア: 入力中のスキルを、ロックオン対象がいる間だけ表示する。同じ拍で始動した複数のスキルはすべて表示する。ロックオンの着脱には毎フレーム追従する
- 装備スキル一覧: 該当スキルの行の拍ステップを更新する
...
sequenceDiagram
    ...
    Guide ->> Guide: 最長一致のスキルだけを表示（未入力なら全スキル）
