...
- 決定（2026-09-22 八幡）: R5 拍の色を統一する（オレンジ = SMG、黄色 = ハンドガン、緑 = アサルト、水色 = リボルバー、青 = ショットガン、紫 = スナイパー）。演出値の表の後に、拍子・色・武器の表を足した。実装の 2 か所（`Assets/Level/Data/Master/InGame/Skill/SkillInputProgressUIConfig.asset:16-38`、`Assets/Level/Prefabs/Master/InGame/InGameCanvas.prefab:1821-1827`）はすでにこの対応である。Issue #2044 では他の表示箇所との食い違いが無いかだけ確かめればよい
- 要確認: なし
...
拍子ごとの色は、武器と合わせて次のとおりに統一する。キルコード入力ガイドの色（`SkillInputProgressUIConfig.asset`の`_normalColor`）とリズムガイドの色（`InGameCanvas.prefab`の`_beatColor`）は、どちらもこの対応になっている。
