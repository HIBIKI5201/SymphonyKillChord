...
- 整合性:
  ...
  - ジャスト倍率: 現行データでは全 6 武器の `_justDamageMultiplier` が 1（`Assets/Level/Data/Master/InGame/Battle/PlayerAttack_Beat*.asset:18`）で、`仕様概要 / リズム判定 / Just判定について` の 200% と食い違う。データの誤り（`_justDamageMultiplier` を 2 に直す必要あり）（BT-01）
  - 決定（2026-09-22 八幡）: Just ダメージ倍率は仕様（200%）が正しく、全 6 武器の `_justDamageMultiplier: 1` はデータの誤り。本ページの本文は倍率の値を書かないため変更なし
- 変更点:
...
- 決定（2026-09-22 八幡）: R6 攻撃硬直は拍だけで決める（固定の秒数をやめる）。「🏗️ クラス」の `AttackIntervalEvaluator` の行に書いた
- 決定（2026-09-22 八幡）: R9 確定ダメージ（下限 50）を実装する。「🏗️ クラス」の `ConfirmedDamage` の行に、プレイヤーのパイプラインに入っていないことを書いた
- 実装の修正が必要: R6 攻撃硬直の固定秒をやめる（Issue #2045、根拠 `Assets/Level/Data/Master/Character/Player.asset:19`）
- 実装の修正が必要: R9 プレイヤーのパイプラインに確定ダメージを入れる（Issue #2047、根拠 `Assets/Level/Data/Master/InGame/Battle/PlayerAttackPipeline.asset:16-29`）
- 要確認: なし
...
## 🏗️ クラス {toggle="true"}
	...
	| **`ConfirmedDamage`** / **`CriticalStep`** / **`WeaponDamageStep`** / **`OutOfRangeDamageStep`** | Application | ダメージ確定・クリティカル抽選・武器倍率とジャスト倍率・射程外減衰の各ステップ。`ConfirmedDamage`（確定ダメージ。下限を下回るダメージを下限に置き換える）はプレイヤーのパイプラインに入っておらず、入れる必要がある（Issue #2047） |
	| **`AttackIntervalEvaluator`** | Application | 攻撃開始からの硬直時間を計測し、攻撃中フラグを管理する。硬直は拍だけで決める仕様で、現状の固定秒（`_attackInterval`）はやめる（Issue #2045） |
