...
- 決定（2026-09-22 八幡）: No.10 Just の判定幅は小節長に対する割合で決まり BPM に比例する（実装どおり）。「📝 ジャスト判定の要件」に判定幅の決まり方と現在値（0.03125 小節、BPM84 で約 89ms、TGS で 1.25 倍に緩和、前後非対称で遅れ側への偏り）の項目を追加した（`Assets/Level/Data/Master/InGame/Music/RhythmJudgmentDefinitionAsset.asset:16-45`、PR #1692）
- 決定（2026-09-22 八幡）: No.11 リズム判定オフセットは判定と表示の両方をずらす。「📥 依存しているもの」の Environment の詳細に、表示（`MusicSyncState`）と判定（`IMusicSyncService`）の両方へ渡す旨を追記した（`Assets/Scripts/Runtime/3.Adaptor/InGame/Music/MusicSyncController.cs:36-41`）。自分の入力への効き方は子ページ ① の【要実機確認】に残した
- 決定（2026-09-22 八幡）: No.2 「📤 依存されているもの」の Stage の詳細 旧: Wave開始時 → 新: ウェーブ開始時
- 決定（2026-09-22 八幡）: R2 リズムゲージのジャスト帯は判定範囲に影響されない統一の太さにする（太さは今と同じ）。「🏗️ クラス」の `ACLikeRhythmGuideView` の行に書いた
- 実装の修正が必要: R2 ジャスト帯を判定幅に関係なく一定の太さで描く（Issue #2042、根拠 `Assets/Scripts/Runtime/4.View/InGame/Music/ACLikeRhythmGuideView.cs:448` `JUST_TIMING_MARKER_WIDTH_SCALE = 1f / 3f`）
- 要確認: なし（ジャスト範囲の拍種ごとの数値は `仕様概要 / リズム判定 / Just判定について` 側で扱う）
...
## 🏗️ クラス {toggle="true"}
	...
	| **`ACLikeRhythmGuideView`** / **`ACLikeRhythmGuideViewModel`** / **`ACLikeRhythmGuideEffectConfig`** | View | AC風リズムガイドのビート表示・判定ゾーン描画とその演出設定。ジャスト帯は判定幅に関係なく一定の太さ（現在の見た目の太さ）で描く。現状は判定幅の1/3で描いている（修正が必要。Issue #2042） |
...
- **`Persistent/Environment`**（任意）
	...
	- *詳細*: `MusicSyncInitializer`がリズム判定オフセット（`RhythmOffsetSeconds`）を取得し、`MusicSyncController`が再生時間へ加える。加えた再生時間を`MusicSyncState`（表示）と`IMusicSyncService`（判定）の両方へ渡すため、判定と表示の両方がずれる。取得できない場合はオフセット0秒で続行する
...
- **`InGame/Stage`**
	...
	- *詳細*: `StageEffectInitializer`がウェーブ開始時の演出（爆発・建物倒壊等）をBGM同期で再生するために利用する
- **`InGame/UI`**
...
- 判定幅は小節長に対する割合で決まり、BPMに比例して変わる。現在の幅はどの拍種も0.03125小節（BPM84で約89ms）で、TGS対応（2026-09-16、PR #1692）で1.25倍に緩和した値である。範囲は拍の位置に対して前後対称ではない。8・4・2・1拍はほぼ遅れ側だけに広がり、6・3拍は前後の両側に広がる（前後の幅は等しくない）
