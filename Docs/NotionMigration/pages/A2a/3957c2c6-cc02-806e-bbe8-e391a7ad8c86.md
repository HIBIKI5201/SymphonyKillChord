...
- 決定（2026-09-22 八幡）: No.1 本文の説明語の「スキル」を「キルコード」に直した（「💡 モジュール概要」「🏗️ クラス」の役割欄、「🔗 モジュール結合」の図のラベル、「📥 依存しているもの」「📤 依存されているもの」の詳細、「🧅レイヤー情報」、「🔌 拡張ポイント」）。モジュール名（Skill・SkillEffect・SkillBuild）、クラス名、「スキルツリー」、子ページのページ名は変えていない
- 決定（2026-09-22 八幡）: No.19 回避はリズムを刻まない（実装どおり）。「② Application」の【要確認】を外し、回避はリズムを刻まない旨を仕様として書いた
- 要確認:
...
> 💡 **モジュール概要**
> 戦闘中のキルコード発動を司るモジュールである。リズムコマンドの入力判定、クールダウン管理、効果の実行、発動状況のUI表示までを担当する。装備キルコードの編成（SkillBuild）とスキルツリーの解放（SkillTree）は、別モジュールへ分離した。
...
## 🏗️ クラス {toggle="true"}
	...
	| クラス名 | レイヤー | 役割・機能 |
	| --- | --- | --- |
	| **`SkillDefinition`** | Domain | キルコードの定義（発動条件・効果・演出）を保持するドメインクラス |
	| **`SkillId`** / **`SkillType`** / **`SkillLevel`** | Domain | キルコードの識別子・種別・レベル |
	| **`SkillPattern`** | Domain | 発動に必要な入力パターン |
	| **`SkillRhythmState`** | Domain | プレイヤーのリズム入力履歴。キルコードごとに持ち、容量はパターン長の2倍 |
	| **`SkillEffectSpec`** / **`SkillEffectType`** / **`SkillEffectParameter`** / **`SkillEffectParameterId`** | Domain | キルコードの効果の定義データと、そのパラメータ |
	| **`SkillEffectParameterGrowth`** / **`SkillEffectParameterGrowthStep`** / **`SkillEffectParameterGrowthType`** | Domain | キルコードレベルによる効果パラメータの成長方式と、レベルアップごとの成長ステップ |
	...
	| **`SkillTemplate`** / **`SkillEffectContext`** | Domain | キルコードの設定データと、効果の発動に必要な情報（対象・プレイヤー・拍・ジャスト成否）をまとめた構造体（置き場は`1.Domain/Player/`） |
	| **`SkillUsecase`** | Application | キルコード発動の判定と実行を扱うユースケース（ファイル名は`SkillUseCase.cs`）。対象を解決できないときは効果を実行せず、発動は成立させる（空撃ち） |
	...
	| **`ISkillRepository`** | Application | キルコード定義の取得抽象 |
	| **`IViewAction`** | Application | キルコードの実行（Effect + Visual）を行うインターフェース。現在は宣言だけで利用箇所が無い |
	| **`SkillExecutionController`** | Adaptor | キルコードの発動を管理するコントローラー。クールダウン・入力履歴・パターン照合・発動・進捗表示の更新をキルコード1つ分受け持つ |
	| **`SkillHitScheduler`** | Application | 連撃キルコードのヒットを時間差で適用するスケジューラ。ダメージのタイミングはこのスケジューラが権限を持ち、エフェクトの生成可否には依存しない |
	| **`SkillHitController`** | Adaptor | 連撃キルコードのヒット進行をViewから受け取り、Applicationへ橋渡しする |
	| **`SkillHitLoopView`** | View | 連撃キルコードのヒット適用を定期更新する（`IGameplayControllable`実装） |
	| **`SkillController`** | Adaptor | 攻撃入力のたびに入力履歴を登録し、装備キルコードごとの`SkillExecutionController`で判定を回す。発動したキルコードのアニメーション・ボイス・武器の表示要求を通知し、通常攻撃のダメージを消すかどうかを呼び出し元へ返す |
	| **`SkillAttackController`** | Adaptor | キルコード専用の攻撃実行アダプター |
	| **`SkillCooldownState`** | Adaptor | キルコードごとのクールダウン完了時刻を管理し発動可否を判定 |
	| **`SkillExecutionResult`** / **`SkillExecutionResultType`** / **`SkillExecutionFailurePolicy`** | Adaptor | 発動結果と、失敗時の扱い |
	| **`SkillTargetResolver`** | Adaptor | キルコード発動時のターゲット解決（Targetモジュールの`TargetAreaQuery`を利用） |
	| **`SkillResultPresenter`** / **`SkillResultDTO`** / **`ISkillResultViewModel`** | Adaptor | 発動結果をViewModel用DTOへ変換して通知 |
	| **`ISkillVisual`** | Adaptor | キルコードのビジュアル演出の契約（`SkillView`が実装） |
	...
	| **`SkillCrosshairProgressController`** | Adaptor (SkillUI) | クロスヘア上の進捗表示の制御。入力中のキルコードを、ロックオン中だけ表示する |
	| **`SkillGuideProgressController`** | Adaptor (SkillUI) | リズムガイド下のキルコード入力ガイドの表示制御。未入力のときは全キルコードの走り出しを、入力中は最も進んでいるキルコードだけを表示する |
	| **`SkillView`** / **`SkillResultView`** / **`SkillResultViewModel`** | View | キルコード発動結果の表示 |
	| **`SkillCrosshairProgressView`** / **`SkillCrosshairStepView`** | View (SkillUI) | クロスヘア進捗の描画 |
	| **`SkillGuideProgressView`** | View (SkillUI) | リズムガイド下のキルコード入力ガイド。次に入力する1拍のキルコードアイコンを、リズムガイドの同じ拍のジャスト位置へ左右対称に出し、その拍の範囲に色帯を付ける。クールダウン中はゲージで残り時間を示す |
	| **`SkillListRowView`** / **`SkillListStepView`** | View (SkillUI) | 装備キルコード一覧の行と拍ステップ表示 |
	| **`SkillBeatVisualSetting`** / **`SkillInputProgressUIConfig`** ほか設定クラス | View (SkillUI) | 拍アイコンの見た目・アニメーションの設定 |
	| **`SkillRepository`** / **`SkillTemplateAsset`** | Infrastructure | キルコード定義（`SkillTemplate`）を保持するScriptableObject。`SkillInitializer`がAddressablesから読み込む |
	| **`SkillDisplayTextFormatter`** / **`SkillEffectDescriptionFormatter`** / **`SkillDisplayText`** | Adaptor (OutGame) | キルコード説明文の整形。アウトゲームの一覧表示で使用 |
...
- **`Music`**
	...
	- *詳細*: リズムコマンドの判定に拍タイミングを使用する。攻撃入力を`RegisterBattleActionHistory`で登録し、リズムタイムアウト（`OnRhythmTimedOut`）で全キルコードの入力進捗をリセットする。BPMから演出と連撃の再生速度を決める
- **`Target`**
	- *依存箇所*: `TargetSystemModuleContainer`（`TargetAreaQuery`を含む）
	- *詳細*: `SkillTargetResolver`が単体・範囲キルコードの対象を解決する
- **`Player`**
	- *依存箇所*: `PlayerModuleContainer`
	- *詳細*: プレイヤーのEntity・Transform・ステータスボーナス・テスト用装備（改造画面を経由しない場合）を取得し、キルコードのアニメーション・ボイスの要求を`PlayerView`へ結線する。`SkillAttackController`がプレイヤーのEntityでキルコード由来の攻撃を実行する
- **`SkillEffect`**
	- *依存箇所*: `SkillEffectExecutorFactory`, `SkillEffectModuleContainer`（`ISkillEffectPlayer`）
	- *詳細*: 効果の実行器を解決テーブルへ登録し、各キルコードの`SkillView`へエフェクト再生ポートを渡す
- **`SkillBuild`**
	- *依存箇所*: `SkillBuildDefinition`, `SkillBuildRepository`
	- *詳細*: 装備中のキルコード構成を取得し、インゲームで発動可能なキルコードを決める。未登録の場合は保存済みの装備から`SkillBuildDefinition`を作って登録する
- **`UI`**
...
- **`Mission`**
	...
	- *詳細*: `InGameMissionInitializer`がキルコード発動を購読し、ミッション条件の達成判定に使用する
- **`Player`**
	- *参照箇所*: `SkillModuleContainer`
	- *詳細*: `PlayerInitializer`がキルコード発動と通常攻撃の結線に使用する。`PlayerAttackController`は攻撃のたびに`SkillController.TryExecuteSkill`を呼び、`PendingAttackEffectService`から命中時効果を取り出す
...
キルコード定義（`SkillDefinition`）、発動に必要な入力パターン（`SkillPattern`）、入力履歴（`SkillRhythmState`）、効果の定義（`SkillEffectSpec`）とクールダウン時間を保持する。

### ② Application

リズムコマンドの正誤判定（`SkillCheckService`）、発動判定と実行（`SkillUsecase`）、効果種別から実行器を引く解決テーブル（`SkillEffectExecutorResolver`）を実装する。<br>判定するのは拍の並びだけで、アクションの種類（`BattleActionType`）は見ない。入力履歴に入るのは攻撃入力だけで、回避はリズムを刻まない。
...
発動結果の表示（`SkillResultView`）と、コマンド入力の進捗表示（クロスヘア進捗・ガイド・装備キルコード一覧）を担当する。拍アイコンの見た目は設定クラスでInspectorから調整できる。

### ⑤ Infrastructure

`SkillRepository`（`SkillTemplateAsset`の集合）がキルコード定義をScriptableObjectとして供給し、`SkillInitializer`がAddressablesから読み込む。

### ⑥ Composition

`SkillInitializer`（Order 450）がキルコード発動まわりを構築し、`SkillModuleContainer`として公開する。

## 🔌 拡張ポイント

| 拡張したいこと | 実装する場所 | 追加登録の要否 |
| --- | --- | --- |
| 新しいキルコードの効果を追加したい | `ISkillEffectExecutor`（Application）を実装したクラスを`Player/SkillEffect`へ追加する | 必要。`SkillEffectType`へ値を足し、`SkillEffectExecutorFactory`へ`resolver.Register(...)`を追記する。自動検出ではないため、登録を忘れると該当キルコードが発動しても効果だけ起きない |
