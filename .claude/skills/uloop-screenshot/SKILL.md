...
V3 boolean options take no value — presence means enabled, absence means disabled. Run `uloop screenshot --help` to confirm current defaults.
...
| `--annotate-elements` | flag | disabled | Annotate interactive UI elements with index labels and interaction hints (A / CLICK, B / DRAG, ...). Only works with `--capture-mode rendering` in PlayMode. |
| `--annotate-raycast-grid` | flag | disabled | Annotate 3D physics raycast candidate points (R1, R2, ...). Only works with `--capture-mode rendering`; uses Camera.main and its culling mask. |
| `--raycast-layer-mask` | string | `""` | Comma-separated physics layer names for `--annotate-raycast-grid`. Hits are limited to layers also visible to Camera.main.cullingMask. When set, dense raycast samples are clustered by clickable GameObject and returned as `Type="PhysicsCollider"` entries in `AnnotatedElements`; multiple colliders on the same GameObject share one entry. Samples blocked by EventSystem `GraphicRaycaster` UI hits are skipped. |
| `--elements-only` | flag | disabled | Return only annotation JSON without capturing a screenshot image. Requires `--annotate-elements` or `--annotate-raycast-grid`, and `--capture-mode rendering` in PlayMode. |
...
- Boolean options are bare flags in V3. Write `--annotate-elements`, `--annotate-raycast-grid`, and `--elements-only`; no `true`/`false` value.
...
```bash
uloop screenshot --capture-mode rendering --annotate-raycast-grid --raycast-layer-mask Default --resolution-scale 1
```
